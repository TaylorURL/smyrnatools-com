 ...
 * between versions with synthetic "patch" entries.
 */
...
/**
 * Displays a timeline of application releases parsed from the bundled CHANGELOG.md.
 * Gaps between version numbers are filled with synthetic "patch" entries.
 * Each version shows as a collapsible card that expands to show per-version changes.
 *
 * @param {Function} onBack - Callback to return to the login screen.
 */
function ChangelogView({ onBack }) {
    const entries = useMemo(() => parseMarkdown(changelogText), [])
