 ...
 */
export default function TruckCoverageHoverCard({
    accentColor,
    bigPour,
    computed,
    customer,
    ...
    // Portal into document.body so the popup escapes the table's stacking
    // context. The schedule's row-stagger animation puts a `transform` on
    // each `<tr>`, which creates a containing block for any
    // `position: fixed` descendant — without the portal the popup would
    // anchor to the row instead of the viewport.
    return createPortal(
        <div className="fixed flex items-center justify-center pointer-events-none" style={{ inset: 0, zIndex: 1000 }}>
            <div
                className="rounded-xl p-5 text-left font-normal normal-case pointer-events-auto"
            ...
            >
                        ...
                        {poolSource.missing > 0 && (
                            <HoverNote>
                                ...
                                <span style={{ color: 'var(--text-tertiary)' }}>
                                    (of {poolSource.rawBase} assigned)
                                </span>
                            </HoverNote>
                        )}
                ...
                {helpInWindow > 0 && (
                    ...
                    >
                        <HoverNote>
                            Trucks returning from other jobs (or help arriving) land at {plantCode} while this pour is
                            still running — they&apos;ll cover later trips.
                        </HoverNote>
                    </HoverRow>
                )}
                ...
                {overbooked && !Number.isFinite(recommendedMoveTime) && (
                    ...
                    >
                        ...
                        <div className="text-[11px] leading-relaxed flex-1" style={{ color: 'var(--text-secondary)' }}>
                            No time later today has the full truck count — {plantCode} will need inbound help from
                            another plant to pour this order at the scheduled rate.
                        </div>
                    </div>
                )}
                ...
                {differsFromDispatch && dispatchTrucks > 0 && (
                    ...
                    >
                        ...
                        <div className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                            Jonel booked <b>{dispatchTrucks}</b>, but our math says you really need <b>{computed}</b>.
                            Go with our number — Jonel&apos;s count is often off.
                        </div>
                    </div>
                )}
            </div>
