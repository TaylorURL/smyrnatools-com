 ...
 *  `customer_contacts` rows so the Call List detail surface always has a
...
import { formatCallListPhone, parsePhoneNumbers } from './CallListUtility'
...
export const mergeCustomerContacts = (
    ...
    for (const entry of parsed) {
        ...
        merged.push({
            ...
            display: override?.phone_display
                ? formatCallListPhone(override.phone_display) || override.phone_display
                : entry.display,
            href: entry.href,
            isHidden: false,
    ...
    for (const row of manualAdditions) {
        merged.push({
            ...
            display: formatCallListPhone(row.phone_display) || row.phone_display,
