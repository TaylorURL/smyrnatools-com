...
interface PlantProductionBlock {
    orders?: Order[]
    [key: string]: unknown
}

type PlantProduction = Record<string, PlantProductionBlock>
...
const formatHHMM = (mins: number): string | null => {
...
const formatDateLabel = (planDate: string | null | undefined): string => {
    if (!planDate) return ''
...
const truckNoun = (count: number): string => (count === 1 ? 'truck' : 'trucks')
...
const resolveDirectLoadJob = (assignment: Assignment, plantProduction: PlantProduction): string | null => {
...
const groupAssignmentsByPlant = (validAssignments: Assignment[]): Map<string, PlantActivity> => {
    const map = new Map<string, PlantActivity>()
    const ensure = (plant: string): PlantActivity => {
...
const buildTruckRows = (driverTimes: DriverTime[], includeLeave: boolean): string[] => {
...
const buildSendRoute = (assignment: Assignment, plantProduction: PlantProduction): string => {
...
const buildReceiveRoute = (assignment: Assignment, plantProduction: PlantProduction): string => {
    const count = parseInt(String(assignment.driverCount), 10) || 0
    const job = resolveDirectLoadJob(assignment, plantProduction)
...
const buildPlantSection = (plantName: string, activity: PlantActivity, plantProduction: PlantProduction): string => {
    const routes = [
...
const buildSummary = (validAssignments: Assignment[], plantProduction: PlantProduction): string => {
    const totalTrucks = validAssignments.reduce((sum, a) => sum + (parseInt(String(a.driverCount), 10) || 0), 0)
    const sentByPlant = new Map<string, number>()
    validAssignments.forEach((a) => {
        const c = parseInt(String(a.driverCount), 10) || 0
    ...
    const directCallouts = new Set<string>()
    validAssignments.forEach((a) => {
 ...
 *
 * @param args.planDate - ISO date the plan is for.
 * @param args.assignments - Help-route assignments.
 * @param args.plantProduction - Plant code -> production block lookup
 *   (used to resolve `forOrderId` to a customer/order tag).
 * @returns Multi-section briefing.
 */
export const buildPlanDispatchText = ({
    assignments = [],
    planDate,
    plantProduction = {}
}: {
    ...
    const validAssignments = assignments.filter(
        (a) => a.fromPlant && a.toPlant && (parseInt(String(a.driverCount), 10) || 0) > 0
    )
