...
export const cleanString = (value) => (value == null ? '' : String(value).trim())
...
export const formatRelativeMinutes = (diffMin) => {
...
/** Minutes a single order ties up trucks for. Trip math: spacing rate ×
...
export const estimatePourMinutes = (order) => {
...
export const flattenPlanOrders = (plantProduction) => {
    const out = []
...
export const buildHelpTransfers = (assignments) => {
    const out = []
...
export const buildInitialPoolByCode = (stats, plantProduction, planDate) => {
    const out = {}
    ;(stats || []).forEach((stat) => {
        if (!stat?.code) return
 ...
 *  the filter is inactive. Shared by Demand and Realtime so the
 *  selection model stays in sync. */
export const resolvePlantFilterCodes = ({ plantFilter, plants, userPlantCode }) => {
...
/** Human-readable label for the active plant-filter selection. Centralized
 *  so Demand and Realtime show the same scope text in headers and pickers. */
export const formatPlantFilterDisplay = ({ plantFilter, plantNameByCode, totalPlantOptions }) => {
