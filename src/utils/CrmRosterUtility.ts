/** Shared constants + tiny helpers for the CRM roster/call-log surface. Lives in
...
export const CRM_ROSTER_SORT_OPTIONS: SortOption[] = [
...
export const formatCrmPhone = (phone: string | null | undefined): string => {
    if (!phone) return ''
...
export const parsePhoneNumbers = (raw: string | null | undefined): ParsedPhone[] => {
    ...
    for (const chunk of chunks) {
        ...
        seen.add(key)
        out.push({
            display: formatCrmPhone(numberPart) || numberPart,
...
export const matchesCrmRosterQuery = (row: CrmRosterRow, query: string | null | undefined): boolean => {
...
const compareWithinTier = (a: CrmRosterRow, b: CrmRosterRow, sortKey: string): number => {
...
export const sortCrmRoster = (rows: CrmRosterRow[], sortKey: string): CrmRosterRow[] => {
    const fresh: CrmRosterRow[] = []
    const recent: CrmRosterRow[] = []
