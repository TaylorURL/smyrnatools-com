...
CREATE TABLE IF NOT EXISTS quality_issues (
    ...
    status          TEXT NOT NULL DEFAULT 'active'
                        CHECK (status IN ('active', 'follow_up', 'holding', 'closed')),
    severity        TEXT
                        CHECK (severity IN ('low', 'medium', 'high', 'critical')),
    cost_to_close   NUMERIC(12, 2),
...
DROP POLICY IF EXISTS quality_issues_all ON quality_issues;
CREATE POLICY quality_issues_all ON quality_issues
...
DROP POLICY IF EXISTS quality_issues_history_all ON quality_issues_history;
CREATE POLICY quality_issues_history_all ON quality_issues_history
    FOR ALL USING (true) WITH CHECK (true);
...
DROP POLICY IF EXISTS quality_issues_comments_all ON quality_issues_comments;
CREATE POLICY quality_issues_comments_all ON quality_issues_comments
    FOR ALL USING (true) WITH CHECK (true);
