...
CREATE
OR REPLACE FUNCTION dispatch_upsert_data(rows jsonb)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    IF
rows IS NULL OR jsonb_typeof(rows) IS DISTINCT FROM 'array' THEN
        RAISE EXCEPTION 'rows must be a JSON array';
END IF;

FOR item IN
SELECT *
FROM jsonb_array_elements(rows) LOOP incoming_sources := ARRAY(
...
INSERT INTO dispatch_data (order_date, order_id, order_num, ticket_num, ticket_id,
...
VALUES ((item ->>'order_date'):: date,
        ...
        incoming_sources) ON CONFLICT (order_date, order_id, ticket_num) DO
UPDATE SET
