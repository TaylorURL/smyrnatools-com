...
const YPH_THRESHOLDS = [
    { min: 6, grade: 'excellent' },
    { min: 4, grade: 'good' },
    { min: 3, grade: 'average' }
] as const
const LOST_THRESHOLDS = [
    { max: 0, grade: 'excellent' },
    { max: 5, grade: 'good' },
    { max: 10, grade: 'average' }
] as const
    ...
    try {
        ...
        switch (endpoint) {
            case 'user-past-due-reports': {
                ...
                const beforeIso =
                    typeof body?.beforeDate === 'string' ? new Date(body.beforeDate).toISOString() : today.toISOString()
                ...
                return jsonResponse(
                    {
                        data: {
                            yph,
                            yphGrade,
                            yphLabel: GRADE_LABELS[yphGrade] ?? '',
                            lost,
                            lostGrade,
                            lostLabel: GRADE_LABELS[lostGrade] ?? ''
                        }
                    },
                    headers
                )
            ...
            case 'plant-production-insights': {
                ...
                const isExcluded = (row: any) =>
                    !row ||
                    Object.keys(row)
                        .filter((k) => k !== 'name' && k !== 'truck_number')
                        .every((k) => (!row[k] && row[k] !== 0) || row[k] === 0)
                let totalLoads = 0,
                    totalHours = 0,
                    totalElapsedStart = 0,
                    totalElapsedEnd = 0,
                ...
                includedRows.forEach((row) => {
                    ...
                    if (!isNaN(loads) && hrs && hrs > 0) {
                        loadsPerHourSum += loads / hrs
                        loadsPerHourCount++
                    }
                    ...
                    if (hrs !== null && hrs > 14)
                        warnings.push({
                            row: rows.indexOf(row),
                            message: `Total Hours is ${hrs.toFixed(2)} (> 14 hours)`
                        })
                })
            ...
            case 'export-csv': {
                const body = await parseBody(req)
                const rows: any[] = Array.isArray(body?.rows) ? body.rows : []
                const operatorOptions: Array<{ value: string; label: string }> = Array.isArray(body?.operatorOptions)
                    ? body.operatorOptions
                    : []
            ...
            case 'week-range': {
                ...
                return jsonResponse(
                    { data: { range: `${formatDateMMDDYY(monday)} through ${formatDateMMDDYY(saturday)}` } },
                    headers
                )
            }
            ...
            case 'save-exclusion-reason': {
                ...
                const { error } = await supabase
                    .from('report_operator_exclusion_reasons')
                    .upsert({ reason, report_id: reportId }, { onConflict: 'report_id' })
            ...
            case 'delete-report': {
                ...
                if (existing.user_id !== userId) {
                    ...
                    if (callerWeight <= 40)
                        return errorResponse('Forbidden: you can only delete your own reports', headers, 403)
                }
