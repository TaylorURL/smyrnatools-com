        ...
        switch (endpoint) {
            ...
            case 'fetch-items-with-profiles': {
                ...
                const { data: items, error: itemsError } = await admin
                    ...
                    .order('created_at', { ascending: false })
                if (itemsError)
                    return errorResponse('Operation failed: ' + (itemsError.message || itemsError.code), headers, 400)
            ...
            case 'fetch-creator-profiles': {
                ...
                const userIds: string[] = Array.isArray(body?.userIds)
                    ? body.userIds.filter((v: any) => typeof v === 'string' && v.trim())
                    : []
            ...
            case 'create': {
                ...
                if (typeof description !== 'string' || !description.trim())
                    return errorResponse('Description is required', headers, 400)
                const id = crypto.randomUUID()
                const now = nowISO()
                const validPriorities = ['none', 'low', 'medium', 'high', 'urgent']
                const safePriority =
                    typeof priority === 'string' && validPriorities.includes(priority) ? priority : 'none'
                const admin = getAdminClient()
            ...
            case 'update': {
                ...
                const updatePayload = {
                    ...
                    responsible_role:
                        typeof item.responsible_role === 'string' && item.responsible_role
            ...
            case 'toggle-completion': {
                ...
                const { error } = await admin
                    ...
                    .update({
                        ...
                        completed_by: newStatus ? auth : null,
            ...
            case 'add-planned-item': {
                ...
                if (typeof listItemId !== 'string' || !listItemId)
                    return errorResponse('List item ID is required', headers, 400)
                if (typeof plannedDate !== 'string' || !plannedDate)
                    return errorResponse('Planned date is required', headers, 400)
                const id = crypto.randomUUID()
                ...
                if (error)
                    return errorResponse(
                        error.code === '23505' ? 'Item already planned for this date' : 'Operation failed',
                        headers,
                        400
                    )
                return jsonResponse({ success: true, id }, headers)
            ...
            case 'clear-planned-items': {
                const auth = await requireAuthenticated(supabase, req, headers)
                if (auth instanceof Response) return auth
                const elevErr = await requireElevated(supabase, auth, headers)
                if (elevErr) return elevErr
                const body = await parseBody(req)
                const { startDate, endDate } = body
                if (!startDate && !endDate)
                    return errorResponse('Date range required for clear operation', headers, 400)
