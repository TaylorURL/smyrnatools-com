    ...
    try {
        ...
        switch (endpoint) {
            ...
            case "update": {
                ...
                if (!id) return errorResponse("Issue ID is required", headers, 400);
                if (!userId) return errorResponse("User ID is required", headers, 400);
                const {data: current, error: currentErr} = await supabase.from(ISSUES_TABLE)
                ...
                const nextRow: Record<string, any> = {
                    cost_to_close: "cost_to_close" in body
                        ? sanitizeCost(body.cost_to_close)
                        : (nextStatus === "closed" ? current.cost_to_close : null),
                ...
                const {data, error} = await supabase.from(ISSUES_TABLE)
                    .update(nextRow).eq("id", id).select().single();
