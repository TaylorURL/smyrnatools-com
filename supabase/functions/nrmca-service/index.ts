    ...
    try {
        ...
        switch (endpoint) {
            ...
            case 'upsert-plant': {
                ...
                if (!plant_code || !plant_label)
                    return errorResponse('plant_code and plant_label are required', headers, 400)
            ...
            // ── Scale CRUD ───────────────────────────────────────────────────
            case 'upsert-scale': {
                const { id, nrmca_plant_id, plant_code, scale_name, scale_type, calibration_interval_days, notes } =
                    body
                if (!nrmca_plant_id || !scale_name)
                    return errorResponse('nrmca_plant_id and scale_name are required', headers, 400)
                const now = new Date().toISOString()
                const payload: any = {
                    nrmca_plant_id,
            ...
            case 'log-renewal': {
                ...
                if (!nrmca_plant_id || !renewed_at)
                    return errorResponse('nrmca_plant_id and renewed_at are required', headers, 400)
                const now = new Date().toISOString()
            ...
            case 'log-calibration': {
                ...
                if (!scale_id || !calibrated_at)
                    return errorResponse('scale_id and calibrated_at are required', headers, 400)
                const now = new Date().toISOString()
                const [{ error: histErr }, { error: updateErr }] = await Promise.all([
